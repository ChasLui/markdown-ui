import { Widget, ParseResult, StreamingParseResult } from './types.js';
...
export class DSLParser {
   ...
   */
  private normalizeArraySyntax(arrayToken: string): string {
  ...
  private parseArray(arrayToken: string): string[] {
    if (!arrayToken.startsWith('[') || !arrayToken.endsWith(']')) {
  ...
  private parseTextInput(tokens: string[]): ParseResult {
    ...
    if (tokens.length > 2) widget.label = tokens[2];
    if (tokens.length > 3) widget.placeholder = tokens[3];
    if (tokens.length > 4) widget.default = tokens[4];

    return { success: true, widget };
  }

  private parseButtonGroup(tokens: string[]): ParseResult {
    if (tokens.length < 3) {
    ...
    try {
      ...
      if (tokens.length > 3) widget.default = tokens[3];
  ...
  private parseSelect(tokens: string[]): ParseResult {
    if (tokens.length < 3) {
  ...
  private parseSelectMulti(tokens: string[]): ParseResult {
    if (tokens.length < 3) {
  ...
  private parseSlider(tokens: string[]): ParseResult {
    if (tokens.length < 5) {
  ...
  private parseForm(tokens: string[], fullInput: string): ParseResult {
    if (tokens.length < 2) {
  ...
  private parseQuiz(tokens: string[], fullInput: string): ParseResult {
    if (tokens.length < 3) {
  ...
  parseStreaming(input: string): StreamingParseResult {
    ...
    if (result.success && result.widget) {
      return {
        complete: true,
        detectedType,
        widget: result.widget as (Partial<Widget> & { type: string })
      };
    }
   ...
   */
  private detectPendingState(input: string): { unclosedBracket: boolean; unclosedQuote: boolean } {
    let inQuotes = false;
    let quoteChar = '';
    let bracketDepth = 0;
  ...
  private parsePartialWidget(
    input: string,
    detectedType: string,
    pending: { unclosedBracket: boolean; unclosedQuote: boolean }
   ...
   */
  private autoCompleteArrays(input: string): string {
    let result = input;
    let bracketDepth = 0;
    let inQuotes = false;
    let quoteChar = '';
    ...
    while (i < lines.length) {
      ...
      if (line.includes(':')) {
        ...
        switch (key) {
          ...
          case 'passingscore':
            const score = parseInt(value);
            if (!isNaN(score)) widget.passingScore = score;
            break;
        }
        i++;
      } else {
   ...
   */
  private parsePartialQuestion(line: string, lineNum: number): any | null {
    ...
    try {
      ...
      if (questionType === 'short-answer') {
        ...
        if (tokens[4]) question.placeholder = tokens[4];
        if (tokens[5]) {
          try {
  ...
  private parsePartialChart(
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (line.includes(':') && !csvStarted) {
        ...
        if (key.toLowerCase() === 'title') widget.title = value;
        if (key.toLowerCase() === 'id') widget.id = value;
...
export function parseDSLStreaming(input: string): StreamingParseResult {
  const parser = new DSLParser();
